...
class TemplatePower extends TemplatePowerParser
{
  ...
  var $prepared;
	 ...
	 * @access private
     */
    function __readDB( $title, $opcion )
    {
        global $conn,$lang;
				$sql="select lt_descripcion,lt_significado from lenguajes_titulos where lt_id=\"$title\" ".
				      "and ls_id=\"$lang\"";
				$recordSet = &$conn->Execute($sql);
				if (!$recordSet)
				{
          //mensaje de error
          mal($conn->ErrorMsg()."  ". __LINE__);
          $conn->disconnect(); die();
        }

				//Si no encuentro el titulo bajo ese lenguaje, busco indefinido bajo el mismo
				// lenguaje
				if($recordSet->RecordCount()!=1)
				{
				  $sql="select lt_descripcion from lenguajes_titulos where lt_id=\"INDEFINIDO\" ".
				       "and ls_id=\"$lang\"";
				  $recordSet = &$conn->Execute($sql);
				  if (!$recordSet)
				  {
            ...
            $conn->disconnect(); die();
					...
					if(!$recordSet->NumRows()) return("ERROR"); 
				}
				switch($opcion)
				{
					case 2:
						return($recordSet->fields["lt_significado"]);
						break;
					default:
						return($recordSet->fields["lt_descripcion"]);
						break;
				}
  ...
  /**********
	 ...
	 * @access public
     */
    function assign( $varname, $value='',$db=0 )
    {		
        if($db) $value=$this->__readDB($value,$db);
        if (is_array( $varname ))
        {
            foreach($varname as $var => $value)
            {
