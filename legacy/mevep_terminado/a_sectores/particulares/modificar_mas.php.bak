...
$fecha_nac=$anio_nac."-".$mes_nac."-".$dia_nac;
 ...
 $sql = "UPDATE `mevep`.`animal` SET `nombre` = '$nombre_mascota', `especie` = '$especie', `raza` = '$raza', `pelaje` = '$pelaje', `tamanio` = '$tamanio', `color` = '$color', `sexo` = '$sexo_mascota', `fecha_nac` = '$fecha_nac' WHERE cod_socio = $cod_socio and cod_animal = $cod_animal";
mysql_query($sql);
