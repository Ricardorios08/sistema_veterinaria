...
$cod_socio_nuevo=$_POST["cod_socio_nuevo"];
...
if ($departamento == ""){
$sql="select departamento from particulares  where cod_socio = $cod_socio";
$result = $db->Execute($sql);
...
if ($sexo == ""){
$sql="select sexo from particulares  where cod_socio = $cod_socio";
$result = $db->Execute($sql);
...
if ($tipo_pago == ""){
$sql="select no_imprimir from particulares  where cod_socio = $cod_socio";
$result = $db->Execute($sql);
...
//}
 ...
 $sql = "UPDATE `pagos` SET `cobrador` = '$cobrador' WHERE `cod_socio` = '$particulares' and estado = 'PENDIENTE'";
mysql_query($sql);


if ($cod_socio_nuevo > 0){
 $sql = "UPDATE particulares SET `cod_socio` = '$cod_socio_nuevo' WHERE cod_socio = $cod_socio";
//mysql_query($sql);
 $sql = "UPDATE `animal_particular` SET `cod_socio` = '$cod_socio_nuevo' WHERE cod_socio = $cod_socio";
 //mysql_query($sql);
 $sql = "UPDATE diagnostico SET `cod_socio` = '$cod_socio_nuevo' WHERE cod_socio = $cod_socio and tipo_soc = 'part'";
 //mysql_query($sql);
}
