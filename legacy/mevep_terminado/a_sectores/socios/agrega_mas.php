...
$fecha_nac=$anio_nac."-".$mes_nac."-".$dia_nac;
 ...
 $sql="select * from animal  ORDER BY cod_animal DESC";
$result = $db->Execute($sql);
$cod_animal=($result->fields["cod_animal"] + 1);


 $sql = "INSERT INTO `animal` ( `cod_socio` , `nombre` , `especie` , `raza` , `pelaje` , `tamanio` , `color` , `sexo` , `fecha_nac` , `historia_clinica` , `cod_animal` , `motivo` , `nuevo` , `nuevos` )  VALUES ('$cod_socio' , '$nombre_mascota' , '$especie' , '$raza' , '$pelaje' , '$tamanio' , '$color' , '$sexo_mascota' , '$fecha_nac' , '$historia_clinica' , '$cod_animal' , '$motivo' , '$nuevo' , '$nuevos')";
mysql_query($sql);
