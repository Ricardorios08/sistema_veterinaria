...
<table width="103%" border="0">
  <tr bgcolor="#CFCFCF" class="Estilo26">
    ...
    <td width="4%" scope="col"><div align="center"><span class="Estilo1">Borrssar</span></div></td>
  </tr><?
...
$total_renglon=$result3->fields["total"];
$iva_renglon=$result3->fields["iva_renglon"];
...
//echo "--------".$iva_renglon = (($precio_actualizado * $iva_normal)/100) * $cantidad;
if ($tipo_iva == 4){
 $iva = 0;
}
else
...
$subtotal;
$desc_factura = round(($subtotal * $porc_dto)/100,2);
$neto_gravado = $subtotal - $desc_factura;
if ($tipo_iva == 4){
$iva = 0;
}else{
$iva =  round(($neto_gravado * 21)/100,2);
}

$total_factura = $subtotal - $desc_factura + $iva;}
else{
$desc_factura = round(($subtotal * $porc_dto)/100,2);
$desc_iva = round(($iva * $porc_dto)/100,2);
$iva = $iva - $desc_iva;
$total_factura = $subtotal - $desc_factura;
$neto_gravado = $total_factura - $iva;
}
...
echo "*".$sql  = "UPDATE `notacredito_encab_temp` SET `bruto` = '$subtotal', `descuento` = '$desc_factura' , `neto_gravado` = '$neto_gravado' , `iva` = '$iva' , `retencion` = '$retencion', `neto` = '$total_factura'  WHERE `tipo_fact` = '$tipo_fact_afectado' AND `nro_factura` = '$nro_factura_nc'";
mysql_query($sql);
